...
<body class="bg-gray-50 font-sans text-gray-800 h-screen flex overflow-hidden">
  ...
  <aside class="w-72 bg-dark text-white flex flex-col hidden md:flex flex-shrink-0 z-20">
    ...
    <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 scrollbar-hide">
      ...
      <a href="verifikasi.php" class="flex items-center gap-3 px-4 py-3 bg-white/10 text-white border border-white/5 rounded-xl font-medium transition-colors">
          ...
          <table class="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr class="bg-gray-50/50 text-gray-500 text-xs uppercase tracking-wider">
                <th class="p-4 pl-6 font-semibold">TGL</th>
                <th class="p-4 font-semibold">Nama Pengguna</th>
                <th class="p-4 font-semibold">NIK & Kontak</th>
            ...
            <tbody class="divide-y divide-gray-100">
              ...
              <tr class="hover:bg-gray-50/50 transition-colors">
                <td class="p-4 pl-6 text-sm text-gray-500 whitespace-nowrap">
                  <?= date('d M Y, H:i', strtotime($pj['tanggal_pengajuan'])) ?>
                </td>
                <td class="p-4">
                  <div class="font-bold text-dark"><?= htmlspecialchars($pj['nama_pengguna']) ?></div>
                </td>
                <td class="p-4 text-sm text-gray-600">
                  <div class="font-medium text-dark"><?= htmlspecialchars($pj['nik']) ?></div>
                  <div class="text-xs text-gray-500"><?= htmlspecialchars($pj['email']) ?><br><?= htmlspecialchars($pj['no_telp']) ?></div>
                </td>
                <td class="p-4">
                  <p class="text-sm text-gray-600 line-clamp-3" title="<?= htmlspecialchars($pj['deskripsi'] ?? '') ?>">
                     <?= nl2br(htmlspecialchars($pj['deskripsi'] ?? '-')) ?>
                  </p>
                </td>
                <td class="p-4">
                  <?php if ($pj['status'] === 'MENUNGGU'): ?>
                    <span class="px-2.5 py-1 bg-yellow-50 text-yellow-600 border border-yellow-200 rounded-md text-xs font-bold tracking-wide">MENUNGGU</span>
                  <?php elseif ($pj['status'] === 'DITERIMA'): ?>
                    <span class="px-2.5 py-1 bg-green-50 text-green-600 border border-green-200 rounded-md text-xs font-bold tracking-wide">DITERIMA</span>
                  <?php else: ?>
                    <span class="px-2.5 py-1 bg-red-50 text-red-600 border border-red-200 rounded-md text-xs font-bold tracking-wide" title="<?= htmlspecialchars($pj['catatan_admin']) ?>">DITOLAK</span>
                  <?php endif; ?>
                </td>
                <td class="p-4 pr-6 text-right">
                  ...
                  <div class="flex items-center justify-end gap-2">
                    <!-- Form Terima -->
                    <form method="POST" action="" class="inline-block" onsubmit="return confirm('Yakin ingin MENERIMA pendaftaran <?= htmlspecialchars($pj['nama_pengguna']) ?>?')">
                      <input type="hidden" name="id_pengajuan" value="<?= $pj['id_pengajuan'] ?>">
                      <input type="hidden" name="id_pengguna" value="<?= $pj['id_pengguna'] ?>">
                      <input type="hidden" name="action" value="terima">
                      <button type="submit" class="px-3 py-1.5 bg-green-100 text-green-700 hover:bg-green-600 hover:text-white rounded shadow-sm text-xs font-bold transition-colors">
                        TERIMA
                      </button>
                    </form>
                    
                    <!-- Form Tolak -->
                    <button type="button" onclick="openTolakModal(<?= $pj['id_pengajuan'] ?>, <?= $pj['id_pengguna'] ?>, '<?= htmlspecialchars($pj['nama_pengguna'], ENT_QUOTES) ?>')" class="px-3 py-1.5 bg-red-100 text-red-700 hover:bg-red-600 hover:text-white rounded shadow-sm text-xs font-bold transition-colors">
                      TOLAK
                    </button>
                  </div>
                  <?php else: ?>
  ...
  <!-- Modal Tolak -->
  <div id="modalTolak" class="fixed inset-0 bg-dark/60 backdrop-blur-sm z-50 hidden flex items-center justify-center opacity-0 transition-opacity duration-300">
    <div class="bg-white max-w-md w-full rounded-2xl shadow-xl overflow-hidden transform scale-95 transition-transform duration-300" id="modalTolakContent">
      <div class="p-6 border-b border-gray-100 flex items-center justify-between">
        <h3 class="text-xl font-bold text-dark flex items-center gap-2">
          <svg class="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          Tolak Pendaftaran
        </h3>
        <button onclick="closeTolakModal()" class="text-gray-400 hover:text-dark transition-colors">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
      </div>
      <form method="POST" action="">
        <div class="p-6">
          <p class="text-gray-600 text-sm mb-4">Anda akan menolak pengajuan dari <strong id="tolakNama" class="text-dark">User</strong>. Silakan isi alasan penolakan jika ada (opsional).</p>
          <input type="hidden" name="id_pengajuan" id="tolakPengajuanId">
          <input type="hidden" name="id_pengguna" id="tolakPenggunaId">
          <input type="hidden" name="action" value="tolak">
          <div>
            <label class="text-sm font-bold text-dark block mb-2">Catatan Admin</label>
            <textarea name="catatan_admin" rows="3" class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-500 text-sm font-medium text-dark" placeholder="Alasan penolakan..."></textarea>
          </div>
        </div>
        <div class="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
          <button type="button" onclick="closeTolakModal()" class="px-5 py-2.5 text-gray-500 font-bold hover:text-dark transition-colors">Batal</button>
          <button type="submit" class="px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold shadow-md transition-colors cursor-pointer">Konfirmasi Tolak</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <script>
    function openTolakModal(id_pengajuan, id_pengguna, nama) {
      document.getElementById('tolakPengajuanId').value = id_pengajuan;
      document.getElementById('tolakPenggunaId').value = id_pengguna;
      document.getElementById('tolakNama').textContent = nama;
      
      const modal = document.getElementById('modalTolak');
      const modalContent = document.getElementById('modalTolakContent');
